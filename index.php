...
$db_selected = $link->select_db('library_database');
...
// <script>console.log('test');</script>
  ...
  if ($link->query($sql)) {
      echo "Database library_database telah dibuat";
  } else {
...
$book_sql = "CREATE TABLE `library_database`.`book` ( 
  `book_id` INT NOT NULL AUTO_INCREMENT , 
  `title` VARCHAR(50) NOT NULL , 
  `author` VARCHAR(50) NOT NULL , 
  `published_date` DATE NOT NULL, 
  PRIMARY KEY (`book_id`)) ENGINE = InnoDB;";
$member_sql = "CREATE TABLE `library_database`.`member` ( 
...
$borrow_sql = "CREATE TABLE `library_database`.`borrow` ( 
